package ru.otus.study.controller.tarantool;
...
@Slf4j
@RestController
@RequestMapping("/memory/dialog")
public class TarantoolDialogController {

    private final DialogTarantoolService dialogService;
    private final JwtService jwtService;

    @Autowired
    public TarantoolDialogController(DialogTarantoolService dialogService, JwtService jwtService) {
    ...
    public ResponseEntity<?> sendMessage(
            ...
            @RequestBody TarantoolDialogController.SendMessageRequest request) {

        try {
            String token = extractToken(authHeader);
            UUID currentUserId = jwtService.extractUserId(token);
            log.info("from_user={} to_user={}",currentUserId,toUserId);
            DialogMessage message = dialogService.saveMessage(
                    currentUserId,
                    toUserId,
                    request.getText()
            );

            return ResponseEntity.ok(new TarantoolDialogController.SendMessageResponse(message.getMessageId()));
